...
class DiceLoss(nn.Module):
    ...
    def __init__(self, num_of_classes, device='cuda', weights=None):
        super(DiceLoss, self).__init__()
        self.C = num_of_classes
        if weights is None:
            self.weights = (torch.ones(self.C) / self.C).to(device)
        else:
...
class CrossEntropyDiceLoss(nn.Module):
    ...
    def __init__(self, num_of_classes, device='cuda', weights_for_class=None, weights=None):
        super(CrossEntropyDiceLoss, self).__init__()
        self.C = num_of_classes
        if weights is None:
            self.weights = (torch.ones(2) / 2).to(device)
        else:
            self.weights = weights / weights.sum()
            
        if weights_for_class is None:
            self.weights_for_class = (torch.ones(self.C) / self.C).to(device)
        else:
